		...
		var AccountAddForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 150,
			labelAlign: 'right',
		...
		var AccountAddWindow = new Ext.Window({
			...
			tools: [{
				...
				handler: function () {
					Ext.Msg.show({
						...
						msg: 'This form is used to insert a new Account.',
				...
				handler: function () {
					if(AccountAddWindow.collapsed)
						AccountAddWindow.expand(true);
					else
						AccountAddWindow.collapse(true);
				}
			}],
			buttons: [  {
				text: '<?php __('Save'); ?>',
