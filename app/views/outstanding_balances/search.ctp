...
,			['Date.name', '<?php __('Date'); ?>']
,			['OutstandingBalance.balance', '<?php __('Balance'); ?>']
,			['OutstandingBalance.created', '<?php __('Created'); ?>']
,			['OutstandingBalance.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var outstandingBalanceSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					outstandingBalanceSearchWindow.close();
...
var outstandingBalanceSearchWindow = new Ext.Window({
	title: '<?php __('Search OutstandingBalance'); ?>',
	modal: true,
	items: outstandingBalanceSearchForm,
	resizable: false,
	width: 550,
	height: 400,
